...
int Init(ESContext *esContext)
{
	...
	model = new Model();
	model->LoadNfg("../Resources/Models/Women1.nfg");
	...
	// Init texture
	texture = new Texture();
	texture->InitTexture2D("../Resources/");
...
void Draw(ESContext *esContext)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, model->GetIboId);
	...
	GLint uvAttribute = glGetAttribLocation(shader->program, "a_uv");
	if (positionAtrribute != -1)
	{
		glEnableVertexAttribArray(positionAtrribute);
		glVertexAttribPointer(positionAtrribute, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)OFFSET_POS);
